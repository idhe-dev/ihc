...
function TemplateDialog(type, template) {
	var Html = $('<div>').load('index.php?v=d&modal=ihc.addByTemplate&plugin=ihc&type=ihc', function () {
				...
				if (isExist == false) {
					...
					if (typeof value.SameCmd == 'undefined') {
						cmd.append($('<label class="col-md-5 control-label" >')
							.text(value.name + " (Type: " + value.configuration.IhcObjectType + ")"));
					} else {
						cmd.append($('<label class="col-md-5 control-label" >')
							.text(value.SameCmd + " (Type: " + value.configuration.IhcObjectType + ")"));
					}
	...
	return $('<div class="col-md-5">')
		.append($('<div class="input-group">')
			...
			.append($('<input type="hidden" class="CmdEqLogicTemplateAttr form-control input-sm" data-l1key="' + index + '" data-l2key="SameCmd">')
				.val(value.SameCmd))
			.append($('<input type="hidden" class="CmdEqLogicTemplateAttr form-control input-sm" data-l1key="' + index + '" data-l2key="IhcObjectType">')
				.val(value.configuration.IhcObjectType)))
		.append($('<div class="input-group">')
